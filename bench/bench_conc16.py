#!/usr/bin/env python3
"""Full concurrency ladder for the batching profile: 2/4/6/8/12/16 streams.

Same protocol as bench_conc.py: non-stream, ignore_eos, forced 1024,
thinking off, 8K unique prompts. Output: eval/conc-ladder.json
"""
...
OUT = Path("/mnt/t5evo/glm53-k35-work/eval/conc-ladder.json")
...
def main():
    ...
    for n in (2, 4, 6, 8, 12, 16):
        ...
        prompts = [sized_prompt(100 + i)[0] for i in range(n)]
        t0 = time.time()
    ...
    print("LADDER:", [(k, v["aggregate_tok_s"] if isinstance(v, dict)
                        else [s["tok_s"] for s in v])

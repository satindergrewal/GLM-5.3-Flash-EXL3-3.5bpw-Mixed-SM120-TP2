...
class Exl3Config(QuantizationConfig):
    ...
    def from_config(cls, config: dict[str, Any]) -> Exl3Config:
        ...
        if config.get("scope") == _GLM53_ROUTED_EXPERTS_SCOPE:
            ...
            bits = config.get("bits")
            if bits != 4 and bits != "mixed_k34_per_tensor":
                raise ValueError(
                    f"GLM-5.3 routed-only EXL3 bits is unsupported: {bits!r}"
                )
            mismatches = {
    ...
    def _require_enforce_eager(self) -> None:
        if self.rank_sliced_metadata is not None or self.r7_routed_experts is not None:
            ...
            # (k35: the R7 projection-tier runtime likewise compiles during
            # the eager profile pass before CUDA graph capture.)
            return
...
class Exl3MoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        import sys as _sys
        print("K35DBG md=", self.quant_config.rank_sliced_metadata, "r7=", getattr(self.quant_config, "r7_routed_experts", None), "unsliced=", self.quant_config.glm53_unsliced_routed_experts, "use_ep=", use_ep, "bits=", self.quant_config.bits, file=_sys.stderr, flush=True)
